...
const ServicesManagement: React.FC = () => {
  ...
  const deleteApprovedGig = async (serviceId: string, title: string) => {
    const confirmed = window.confirm(
      `Delete accepted gig "${title}"?\n\nThis removes it from marketplace listing.`
    );
    if (!confirmed) return;

    setBusy(serviceId);
    try {
      const response = await fetch(API_ENDPOINTS.serviceById(serviceId), {
        method: 'DELETE'
      });

      const json = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(json.error || 'Failed to delete gig');
      }
          ...
          <div className="space-y-4">
            {approvedList.map(service => (
              <div key={`${service.userId}-${service.id}`} className="bg-white rounded-2xl p-4 shadow border border-gray-100">
                ...
                <div className="mt-3">
                  <button
                    onClick={() => deleteApprovedGig(String(service.id), service.title || 'Untitled gig')}
                    disabled={busy === service.id}
                    className="w-full bg-red-500 text-white px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-red-600 disabled:opacity-50"
                  >
                    {busy === service.id ? 'Deleting...' : 'Delete Gig'}
                  </button>
                </div>
              </div>
            ))}
...
const DisputesManagement: React.FC = () => {
  ...
  const loadDisputes = async (keepSelection = true) => {
    try {
      const resp = await fetch(API_ENDPOINTS.disputes);
      const json = await resp.json();
      const rawItems = Array.isArray(json.disputes) ? json.disputes : [];
      const normalized = rawItems.map((d: any) => {
        const tradeId = String(d.tradeId || d.trade_id || d.id || '');
        const trade = trades.find(t => String(t.id) === tradeId);
        return {
          ...d,
          id: String(d.id || `dispute_${tradeId}`),
          tradeId,
          status: String(d.status || 'OPEN').toUpperCase(),
          description: d.description || trade?.description || 'Trade dispute',
          amount: d.amount ?? trade?.amount ?? 0,
          currency: d.currency || trade?.currency || 'USDT',
          buyer: d.buyer || trade?.buyer || null,
          seller: d.seller || trade?.seller || null,
          createdAt: d.createdAt || d.created_at || trade?.createdAt || new Date().toISOString(),
          disputeMessages: Array.isArray(d.disputeMessages) ? d.disputeMessages : []
        };
      });
      setDisputes(normalized);
      if (!keepSelection && normalized.length > 0) {
        setSelectedDisputeId(normalized[0].id);
      }
      if (keepSelection && selectedDisputeId && !normalized.some((d: any) => d.id === selectedDisputeId)) {
        setSelectedDisputeId(normalized[0]?.id || null);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!selectedDispute?.tradeId) return;
    (async () => {
      try {
        const resp = await fetch(API_ENDPOINTS.disputeMessagesByTrade(selectedDispute.tradeId));
        const json = await resp.json();
        if (!json.ok) return;
        const messages = Array.isArray(json.messages) ? json.messages : [];
        setDisputes(prev =>
          prev.map(d => d.id === selectedDispute.id ? { ...d, disputeMessages: messages } : d)
        );
      } catch (error) {
        console.error('Failed to load dispute messages:', error);
      }
    })();
  }, [selectedDispute?.id, selectedDispute?.tradeId]);
  ...
  const filteredDisputes = useMemo(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(d =>
        d.tradeId?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    if (filterStatus !== 'all') {
      if (filterStatus === 'open') filtered = filtered.filter(d => d.status === 'OPEN');
      if (filterStatus === 'resolved') filtered = filtered.filter(d => d.status === 'RESOLVED');
      if (filterStatus === 'escalated') filtered = filtered.filter(d => d.status === 'ESCALATED');
    }
    
    return filtered;
  }, [disputes, searchQuery, filterStatus]);

  const sendDisputeMessage = async (content: string, type: 'TEXT' | 'SYSTEM' | 'EVIDENCE' = 'TEXT') => {
    if (!selectedDispute?.tradeId || !content.trim()) return;
    setSubmittingMessage(true);
    try {
      const payload = {
        id: `dm_admin_${Date.now()}`,
        trade_id: selectedDispute.tradeId,
        sender_id: 'admin',
        type,
        content: content.trim()
      };
      const resp = await fetch(API_ENDPOINTS.disputeMessagesSend, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
  ...
  const uploadEvidenceAndAttach = async (file: File) => {
    if (!selectedDispute?.tradeId || !file) return;
    setUploadingEvidence(true);
    try {
      const formData = new FormData();
      formData.append('file', file);
      const resp = await fetch(API_ENDPOINTS.upload, { method: 'POST', body: formData });
      const json = await resp.json().catch(() => ({}));
      if (!resp.ok || !json.ok) {
        throw new Error(json.error || 'Failed to upload evidence');
      }
      const evidenceText = `Evidence uploaded: ${json.url || json.secure_url || 'file uploaded'}`;
      await sendDisputeMessage(evidenceText, 'EVIDENCE');
    } catch (error: any) {
  ...
  const resolveDispute = async (dispute: any, resolutionText: string, winner: 'buyer' | 'seller') => {
    if (!dispute?.tradeId) return;
    setBusy(dispute.id);
    try {
      const newStatus = winner === 'buyer' ? EscrowStatus.CANCELLED : EscrowStatus.COMPLETED;
      const statusResp = await fetch(API_ENDPOINTS.tradeStatus(dispute.tradeId), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      await sendDisputeMessage(`Dispute resolved: ${resolutionText}. Winner: ${winner}.`, 'SYSTEM');
      updateTradeStatus(dispute.tradeId, newStatus);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4">
          {loading && <div className="bg-white rounded-2xl p-4 shadow border border-gray-100 text-gray-600">Loading disputes...</div>}
          {!loading && filteredDisputes.length === 0 && (
            <div className="bg-white rounded-2xl p-4 shadow border border-gray-100 text-gray-600">No disputes found.</div>
          )}
        {filteredDisputes.map(dispute => (
            <button
              key={dispute.id}
              onClick={() => setSelectedDisputeId(dispute.id)}
              className={`w-full text-left bg-white rounded-2xl p-4 shadow border transition-all ${
                selectedDisputeId === dispute.id ? 'border-blue-400 ring-2 ring-blue-100' : 'border-gray-100 hover:shadow-md'
              }`}
            >
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                  <h3 className="text-base font-semibold text-gray-900 mb-1 truncate">{dispute.description}</h3>
                  <p className="text-xs text-gray-500 mb-2">Trade: {dispute.tradeId}</p>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-lg border border-gray-100 min-h-[520px]">
          {!selectedDispute && (
            <div className="h-full flex items-center justify-center text-gray-500">
              Select a dispute to moderate its chat and resolve it.
            </div>
          )}
          ...
          {selectedDispute && (
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-semibold text-gray-900">{selectedDispute.description}</h3>
                  <p className="text-sm text-gray-600">
                    Trade {selectedDispute.tradeId} • {selectedDispute.amount} {selectedDispute.currency}
                  </p>
                </div>
                <button
                  onClick={() => loadDisputes(true)}
                  className="bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 px-3 py-2 rounded-lg text-sm font-medium"
                >
                  Refresh Chat
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div className="bg-gray-50 rounded-xl p-3">
                  <div><span className="text-gray-500">Buyer:</span> <span className="font-medium text-gray-900">{selectedDispute.buyer?.username || 'Unknown'}</span></div>
                  <div className="mt-1"><span className="text-gray-500">Seller:</span> <span className="font-medium text-gray-900">{selectedDispute.seller?.username || 'Unknown'}</span></div>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <div><span className="text-gray-500">Created:</span> <span className="font-medium text-gray-900">{new Date(selectedDispute.createdAt).toLocaleString()}</span></div>
                  <div className="mt-1"><span className="text-gray-500">Status:</span> <span className="font-medium text-gray-900">{selectedDispute.status}</span></div>
                </div>
              </div>

              <div className="bg-gray-50 p-4 rounded-xl">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-semibold text-gray-900">Dispute Chat</h4>
                  <span className="text-xs text-gray-500">{(selectedDispute.disputeMessages || []).length} messages</span>
                </div>
                <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                  {(selectedDispute.disputeMessages || []).length === 0 && (
                    <div className="text-sm text-gray-500">No messages yet.</div>
                  )}
                  {(selectedDispute.disputeMessages || []).map((msg: any) => {
                    const isAdmin = String(msg.sender_id || msg.senderId) === 'admin';
                    return (
                      <div key={msg.id} className={`rounded-lg p-3 ${isAdmin ? 'bg-blue-50 border border-blue-100' : 'bg-white border border-gray-200'}`}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-medium text-sm text-gray-900">{isAdmin ? 'Admin' : (msg.sender_id || msg.senderId || 'User')}</span>
                          <span className="text-xs text-gray-500">
                            {new Date(msg.created_at || msg.createdAt || msg.timestamp || Date.now()).toLocaleString()}
                          </span>
                        </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) uploadEvidenceAndAttach(file);
                    }}
                    className="text-sm"
                  />
                  <span className="text-xs text-gray-500">{uploadingEvidence ? 'Uploading evidence...' : 'Attach screenshot/evidence'}</span>
                </div>
                <textarea
                  value={newMessage}
                  onChange={e => setNewMessage(e.target.value)}
                  placeholder="Send moderation message to dispute chat..."
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                  rows={3}
                />
                <div className="flex gap-2">
                  <button
                    onClick={() => sendDisputeMessage(newMessage, 'TEXT')}
                  ...
                  <button
                    onClick={() => setNewMessage('Please provide clear evidence (screenshots, transaction IDs, and timestamps) to help resolution.')}
              ...
              <div className="border-t pt-4 space-y-3">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => resolveDispute(selectedDispute, moderationNote || 'Refund approved based on evidence', 'buyer')}
                    disabled={busy === selectedDispute.id}
                    className="flex-1 bg-green-500 text-white px-4 py-3 rounded-lg font-medium hover:bg-green-600 disabled:opacity-50"
                  >
                    Resolve in Favor of Buyer
                  </button>
                  <button
                    onClick={() => resolveDispute(selectedDispute, moderationNote || 'Service delivered as agreed', 'seller')}
                    disabled={busy === selectedDispute.id}
                    className="flex-1 bg-purple-500 text-white px-4 py-3 rounded-lg font-medium hover:bg-purple-600 disabled:opacity-50"
                  >
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">${stats.pendingPayouts?.toFixed(2) || '0.00'}</span>
            </div>
            <div className="flex justify-between">
...
const TradesManagement: React.FC = () => {
  ...
  const contactTradeParty = async (trade: any, role: 'buyer' | 'seller') => {
    const username = role === 'buyer' ? trade?.buyer?.username : trade?.seller?.username;
    const message = window.prompt(`Message ${role} ${username ? `(@${username})` : ''}:`);
    if (!message || !message.trim()) return;

    const busyKey = `${trade.id}_${role}`;
    setContactBusy(busyKey);
    try {
      const resp = await fetch(API_ENDPOINTS.adminContactUser, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tradeId: trade.id,
      ...
      alert(`Failed to contact ${role}: ${error?.message || 'Unknown error'}`);
    } finally {
      setContactBusy(null);
    }
  };
                ...
                <tr key={trade.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        onClick={() => contactTradeParty(trade, 'buyer')}
                        disabled={contactBusy === `${trade.id}_buyer`}
                      ...
                      <button
                        onClick={() => contactTradeParty(trade, 'seller')}
                        disabled={contactBusy === `${trade.id}_seller`}
                        className="text-xs bg-purple-50 text-purple-700 hover:bg-purple-100 px-2 py-1 rounded border border-purple-200 disabled:opacity-50"
                      >
                        Contact Seller
                      </button>
                      <select
...
const WithdrawalsManagement: React.FC = () => {
  ...
  const filteredWithdrawals = useMemo(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(w =>
        (w.userId || w.user_id || '')?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const processWithdrawal = async (id: string, type: 'approve' | 'reject') => {
    ...
    try {
      ...
      const requestBody: Record<string, string> = { id };
      if (type === 'approve') {
        const txId = window.prompt('Optional: Enter blockchain transaction hash');
        const transferProofUrl = window.prompt('Optional: Attach transfer proof screenshot URL');
        if (txId) requestBody.txId = txId;
        if (transferProofUrl) requestBody.transferProofUrl = transferProofUrl;
      } else {
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900">Amount</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900">Address / Network</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredWithdrawals.map(withdrawal => (
                <tr key={withdrawal.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-900">{withdrawal.userId || withdrawal.user_id}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-gray-500 mt-1">{withdrawal.network || '-'}</div>
                    {withdrawal.transferProofUrl && (
                      <a href={withdrawal.transferProofUrl} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline">
                        View proof
                      </a>
                    )}
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900">Fee: ${Number(withdrawal.fee || 1).toFixed(2)}</div>
                    <div className="text-xs text-gray-500">Net: {Number(withdrawal.amountAfterFee ?? withdrawal.amount).toFixed(4)} {withdrawal.currency}</div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900">
                      {new Date(withdrawal.createdAt || withdrawal.created_at || Date.now()).toLocaleDateString()}
                    </div>
                  </td>
                  <td className="px-6 py-4">

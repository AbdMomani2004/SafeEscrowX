...
const NotificationPanel: React.FC<{ 
  ...
  return (
    <div className="absolute top-16 right-0 w-80 bg-surface rounded-2xl shadow-lg border border-border-color z-50 animate-fade-in-down">
...
export const Header: React.FC<{ 
  ...
  return (
    <div className="p-4 text-white flex items-center justify-between sticky top-0 bg-background/80 backdrop-blur-lg z-20 border-b border-border-color">
        <h1 className="text-2xl font-bold">{title}</h1>
         <div className="flex items-center space-x-2">
            {children}
            <div className="relative">
                 <button onClick={() => setPanelOpen(!panelOpen)} className="bg-surface p-3 rounded-full hover:bg-surface-alt transition-colors relative">
...
export const BottomNav: React.FC<BottomNavProps> = ({ activeView, setActiveView, setCreatingNew }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-24 bg-surface/80 backdrop-blur-lg border-t border-border-color">
...
const NavItem: React.FC<{item: { view: View, icon: React.FC<any> }, isActive: boolean, onClick: () => void}> = ({ item, isActive, onClick }) => {
    ...
    return (
        <button onClick={handleClick} className="flex flex-col items-center justify-center w-16 h-16 rounded-2xl transition-all duration-300">
          <div className={`relative transition-all duration-300 ${isActive ? 'text-primary' : 'text-text-body'}`}>

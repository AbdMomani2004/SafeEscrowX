...
const PayoutsTable: React.FC = () => {
    ...
    const act = async (id: string, type: 'approve' | 'reject') => {
        ...
        try {
            ...
            setItems(prev => prev.map(w => w.id === id ? { ...w, status: type.toUpperCase() } : w));
        } finally {
            setBusy(null);
        }
    };
        ...
        <div className="space-y-3">
            {items.map(w => (
                <div key={w.id} className="bg-surface p-4 rounded-2xl border border-border-color">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="font-semibold">{w.amount} {w.currency} → {w.address}</p>
                            <p className="text-xs text-text-body">User: {w.userId} • {new Date(w.createdAt).toLocaleString()}</p>
                        </div>

...
export const withdrawals = {
  create: async (withdrawalData) => {
    ...
    const query = `
      ...
      RETURNING *
    `;
    const result = await pool.query(query, [id, user_id, amount, currency, address]);
    return result.rows[0];
  },
  ...
  updateStatus: async (id, status, processed_at = null, reason = null) => {
    const query = `
      UPDATE withdrawals 
      SET status = $1, processed_at = $2, reason = $3, updated_at = CURRENT_TIMESTAMP
      WHERE id = $4
      RETURNING *
    `;
    const result = await pool.query(query, [status, processed_at, reason, id]);
    return result.rows[0];
  },

  getAll: async () => {
    const query = 'SELECT * FROM withdrawals ORDER BY created_at DESC';
    const result = await pool.query(query);
    return result.rows;
  }
};

...
const StarDisplay: React.FC<{ rating: number, className?: string }> = ({ rating, className = '' }) => {
    ...
    return (
        <div className={`flex items-center space-x-0.5 text-yellow-400 ${className}`}>
...
const ReviewCard: React.FC<{ review: Review; reviewer: User | undefined; trade: Trade; onViewTrade: (tradeId: string) => void; }> = ({ review, reviewer, trade, onViewTrade }) => {
    ...
    return (
        <div className="bg-surface p-4 rounded-2xl border border-border-color">
...
export const CreateEscrowScreen: React.FC<ScreenProps> = ({ setCurrentView, setCreatingNew, selectedUserId, currentUser, prefillTrade }) => {
    ...
    // Fixed fee system: always 1%
    const fee = useMemo(() => {
        const baseAmount = parseFloat(amount) || 0;
        return baseAmount * 0.01; // Fixed 1% fee
    }, [amount]);
...
export const DepositScreen: React.FC<Pick<ScreenProps, 'setCurrentView' | 'showToast' | 'selectedTradeId'>> = ({ setCurrentView, showToast, selectedTradeId }) => {
    ...
    const verifyPayment = async () => {
        ...
        tg?.HapticFeedback.notificationOccurred('success');
             ...
             setTimeout(() => {
                ...
                showToast("Payment not found. Please ensure you've sent the exact amount to the address.");
            }
        } catch (error) {
            setVerificationStatus('failed');
    ...
    // Fixed fee system: always 1%
    const fee = trade.amount * 0.01; // Fixed 1% fee
    ...
    return (
        <div className="p-4 text-white text-center flex flex-col h-full">
            ...
            <p className="text-xs text-text-body mb-6">Fee: 1% (fixed)</p>
                        ...
                        {verificationStatus === 'failed' && (
                            <div className="bg-red-500/20 border border-red-500/30 rounded-xl p-3">
                                <p className="text-red-300 text-sm text-center">
                                    Payment not found. Please ensure you've sent exactly {total.toFixed(4)} {trade.currency} to the address above.
                                </p>
                            </div>
                        )}
                        <p className="text-xs text-text-body text-center">
                            After sending payment, click "Verify Payment" to check the blockchain for your transaction.
                        </p>
                    </div>
                )}
            </div>
...
const ServiceCard: React.FC<{ service: Service; user: User; onClick: () => void }> = ({ service, user, onClick }) => {
    return (
        <div onClick={onClick} className="bg-surface p-4 rounded-2xl flex flex-col justify-between cursor-pointer border border-border-color hover:border-primary transition-colors">
            <div>
...
const WithdrawFundsModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (req: { amount: number; address: string; currency: Currency }) => void;
    balances: Record<Currency, number>;
    ...
    const handleSubmit = () => {
        ...
        setError(null);
        onSubmit({ amount: numericAmount, address, currency });
    };
...
export const ProfileScreen: React.FC<ScreenProps> = ({setCurrentView, currentUser, showToast}) => {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const json = await resp.json();
                const mine = (Array.isArray(json.withdrawals) ? json.withdrawals : []).filter((w: any) => w.userId === currentUser.id);
    ...
    const handleWithdrawSubmit = async (req: { amount: number; address: string; currency: Currency }) => {
        try {
                        await fetch(API_ENDPOINTS.withdrawalRequest, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: currentUser.id, amount: req.amount, currency: req.currency, address: req.address })
            });
            setIsWithdrawModalOpen(false);
            showToast("Withdrawal request submitted successfully!");
        } catch (e) {
            showToast("Failed to submit withdrawal. Try again.");
        }
    };
                ...
                <div className="p-4">
                    <div className="flex flex-col items-center">
                        ...
                        <div className="flex items-center space-x-2 mt-2 bg-surface px-4 py-2 rounded-full border border-border-color">
                            ...
                            <span className="text-lg font-bold text-white">{(Number(user.rating) || 0).toFixed(1)}</span>
                            <span className="text-text-body">({receivedReviews.length} reviews)</span>
                        </div>
                    </div>
                    ...
                    <h2 className="text-xl font-bold self-start mt-8 mb-4">Feedback</h2>
                    <div className="w-full space-y-3">
